...
rep = """ SET FOREIGN_KEY_CHECKS=0; SET @TRIGGER_CHECKS = FALSE; TRUNCATE TABLE `LocationCategory`; """
sql = """ SELECT * FROM location_certificate_categories; """
...
try:
    ...
    try:
        ...
        for ROW in resultset:
            #print(row)
            sql = str.format("INSERT INTO `LocationCategory` VALUES({0}, {1}, {2}, {3}, {4});",
                          'UUID()', getValue(ROW[0]), getValue(ROW[1]), getValue(ROW[2]), getValue(ROW[2]))
